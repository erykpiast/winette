...
describe('Production Configuration', () => {
  ...
  describe('Environment Validation', () => {
    it('should throw error when API keys are missing', () => {
      delete process.env.ANTHROPIC_API_KEY;
      delete process.env.OPENAI_API_KEY;

      expect(() => configureForProduction()).toThrow(
        'Missing required environment variables: ANTHROPIC_API_KEY, OPENAI_API_KEY',
      );
    });

    it('should throw error when only ANTHROPIC_API_KEY is missing', () => {
      delete process.env.ANTHROPIC_API_KEY;

      expect(() => configureForProduction()).toThrow('Missing required environment variables: ANTHROPIC_API_KEY');
    });

    it('should throw error when only OPENAI_API_KEY is missing', () => {
      delete process.env.OPENAI_API_KEY;

      expect(() => configureForProduction()).toThrow('Missing required environment variables: OPENAI_API_KEY');
    });

    it('should configure successfully when all keys are present', () => {
      expect(() => configureForProduction()).not.toThrow();
    });
  });
  ...
  describe('Auto Configuration', () => {
    it('should use production config when NODE_ENV is production', () => {
      ...
      expect(() => autoConfigurePipeline()).not.toThrow();
    });

    it('should use production config when NODE_ENV is staging', () => {
    ...
    it('should use development config when NODE_ENV is not set', () => {
      expect(() => autoConfigurePipeline()).not.toThrow();
    });
  });
});
