...
import { runImageGenerate, runRefine, runRender } from '../lib/langchain-chains.js';
import { configurePipeline, MockImageAdapter, MockVisionRefiner, withRetry } from '../lib/langchain-pipeline.js';
...
describe('LangChain Pipeline', () => {
  ...
  describe('Mock Adapters', () => {
    it('should generate mock images', async () => {
      ...
      expect(result).toEqual({
        ...
        url: 'https://example.com/generated/test-prompt.png',
        width: 800,
        height: 600,
      });
    });
  ...
  describe('Chain Functions with Mock LLM', () => {
    ...
    it('should generate images using adapter', async () => {
      ...
      expect(result.url).toContain('hero-image.png');
